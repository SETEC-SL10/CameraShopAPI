...
@Repository
public interface NewCameraFormDAO{
	
	String F_ProductForm = "SELECT * FROM view_camera_new_form LIMIT #{limit} OFFSET #{page} * #{limit}";
	
	String F_A_Image = "SELECT * FROM view_camera_new_image_form WHERE product_id = #{ProID} AND color_id = #{ColorID} AND status = true";
